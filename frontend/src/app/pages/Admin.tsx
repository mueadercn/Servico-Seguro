        ...
        <div className="p-6 flex-1">
                          ...
                          <tbody>
                            {orcs.slice(0, 10).map((o: any) => (
                              <tr key={o.id} className="hover:bg-slate-50/50 cursor-pointer" onClick={() => abrirOrc(o)}>
                                ...
                                <td className={td + " text-xs text-muted-foreground"}>{o.servicos?.titulo || o.servicos?.categorias?.nome || '—'}</td>
                                <td className={td}>{statusBadge(o.status)}</td>
                                <td className={td + " max-w-xs"}>
                                  {o.resumo_anamnese ? (
                                    <span className="text-xs text-muted-foreground line-clamp-2">{o.resumo_anamnese}</span>
                                  ) : (
                                    <span className="text-xs text-muted-foreground italic">Em andamento...</span>
                                  )}
                                </td>
                                <td className={td + " text-xs text-muted-foreground whitespace-nowrap"}>{timeAgo(o.criado_em)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                      ...
                      {getLeadsDaFase(faseSelecionada).map((orc: any) => (
                        <div key={orc.id}
                          ...
                          <div className="p-5">
                            ...
                            <div className={`rounded-xl p-3 mb-3 ${orc.resumo_anamnese ? 'bg-green-50 border border-green-100' : 'bg-slate-50 border border-border'}`}>
                              <div className="flex items-center gap-1.5 mb-1.5">
                                ...
                                <span className="text-xs font-bold text-success uppercase tracking-wider">Resumo da IA</span>
                              </div>
                              <p className="text-xs text-foreground leading-relaxed">
                                {orc.resumo_anamnese || 'Anamnese em andamento — resumo disponível após conclusão.'}
                              </p>
                            </div>
                          ...
                          <div className="px-5 py-3 bg-slate-50 border-t flex items-center gap-2">
                            ...
                            {faseSelecionada.id === 'qualificado' && (
                              <button
                                onClick={e => e.stopPropagation()}
                                className="ml-auto flex items-center gap-1.5 text-xs bg-success text-white px-3 py-1.5 rounded-lg hover:bg-success/90 transition-colors">
                                <UserPlus className="h-3.5 w-3.5" /> Vincular prestador
                              </button>
                            )}
          ...
          {!loading && aba === 'prestadores' && (
            <div className="bg-white rounded-2xl border overflow-hidden">
              <div className="overflow-x-auto">
                <table className={tbl}>
                  <thead><tr>{['Nome', 'Email', 'Telefone', 'Cidade', 'Status', 'Verificado', 'Ação'].map(h => <th key={h} className={th}>{h}</th>)}</tr></thead>
                  <tbody>
          ...
          {!loading && aba === 'servicos' && (
            <div className="bg-white rounded-2xl border overflow-hidden">
              <div className="overflow-x-auto">
                <table className={tbl}>
                  <thead><tr>{['Título', 'Categoria', 'Prestador', 'Status'].map(h => <th key={h} className={th}>{h}</th>)}</tr></thead>
                  <tbody>
          ...
          {!loading && aba === 'config' && (
            <div className="bg-white rounded-2xl border p-6">
              <h2 className="font-bold text-primary mb-4">Configurações do Sistema</h2>
              <div className="space-y-3">
                {(dados.configs || []).map((c: any) => (
                  <div key={c.chave} className="flex items-center gap-3 py-3 border-b border-border last:border-0">
                    <div className="flex-1">
                      <div className="font-semibold text-sm">{c.chave}</div>
                      {c.descricao && <div className="text-xs text-muted-foreground">{c.descricao}</div>}
                    </div>
                    <div className="text-sm text-muted-foreground font-mono">
                      {c.chave.includes('key') || c.chave.includes('senha') ? '••••••••' : c.valor?.substring(0, 30)}
                    </div>
                  </div>
                ))}
              </div>
