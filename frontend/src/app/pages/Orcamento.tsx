...
import { ArrowLeft, Send, Camera, CheckCircle2 } from 'lucide-react';
...
export function Orcamento() {
  ...
  async function iniciar() {
    if (contratante) {
      ...
      setEtapa('anamnese');
      const h = [{ role: 'assistant', content: `Olá, ${contratante.nome}! Me conta o que você precisa.` }];
      setHistorico(h);
  ...
  async function enviar() {
    ...
    if (etapa === 'nome') {
      setNomeCliente(txt);
      setEtapa('telefone');
      addMsg('ia', `Prazer, **${txt}**! Qual o seu WhatsApp?`);
      return;
    }
    if (etapa === 'telefone') {
      setTelefone(txt);
    ...
    if (etapa === 'anamnese') {
      ...
      try {
        ...
        addMsg('ia', resp);
        if (idx >= fallbacks.length - 1) setTimeout(() => gerarOrc('Serviço solicitado pelo cliente', []), 1500);
      }
      setLoading(false);
    }
  }

  async function gerarOrc(resumo: string, hist: any[]) {
    const codigo = 'ORC-' + new Date().getFullYear() + '-' + String(Date.now()).slice(-5);
    setOrcCodigo(codigo);
    try {
      const result = await apiCall('/api/orcs', {
        method: 'POST',
        body: { servico_id: servicoId, nome_cliente: nomeCliente || contratante?.nome, telefone_cliente: telefone || contratante?.telefone, canal: 'site', servico_nome: servicoNome }
      });
      if (result.ok) setOrcId(result.orc.id);
    } catch (e) { console.warn(e); }
    setEtapa('concluido');
    addMsg('sistema', `✅ ORC gerado: ${codigo}`);
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50">
      <div className="bg-primary px-4 py-3 flex items-center gap-3">
        ...
        <div className="w-9 h-9 rounded-full bg-success/20 flex items-center justify-center text-lg">🛡️</div>
        ...
        {msgs.map((m, i) => {
          ...
          if (m.role === 'concluido') return (
            <div key={i} className="bg-gradient-to-br from-primary to-[#0d2847] rounded-2xl p-6 text-center text-white">
              ...
              <div className="text-white/70 text-sm mb-1">Código: <strong className="text-success">{m.codigo}</strong></div>
              <div className="text-white/50 text-xs mt-2 leading-relaxed">{m.content}</div>
              <Link to="/" className="inline-block mt-4 bg-success text-white px-6 py-2 rounded-xl text-sm font-bold">← Voltar ao site</Link>
            </div>
          );
