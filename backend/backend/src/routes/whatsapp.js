...
router.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    console.log(`[WhatsApp] Evento: ${payload.event}`);

    // Filtrar: só mensagens recebidas
    ...
    // Extrair número — remoteJidAlt tem formato s.whatsapp.net
    ...
    if (!numero || !texto) {
      console.log('[WhatsApp] Ignorado — sem numero ou texto');
      return;
    }
    ...
    // Processar
    await processarMensagem(numero, texto, data);
...
async function processarMensagem(numero, texto, msgRaw) {
  console.log(`[PROC] Iniciando processamento para ${numero}`);
  
  // 1. Identificar ORC ativo para esse número
  const orc = await buscarOrcAtivo(numero);
  console.log(`[PROC] ORC encontrado: ${orc ? orc.codigo + ' status:' + orc.status : 'NENHUM'}`);
  ...
  if (!orc) {
    ...
    if (ids.temIdentificador) {
      ...
      if (novoOrc) {
        console.log(`[PROC] Novo ORC criado: ${novoOrc.codigo}`);
...
async function handleAnamnese(orc, numero, texto) {
  console.log(`[ANAMNESE] Iniciando para ORC ${orc.codigo}`);
  ...
  // Chamar IA
  console.log(`[ANAMNESE] Chamando IA... cat=${orc.servicos?.categorias?.nome} servico=${orc.servicos?.titulo}`);
  ...
  console.log(`[ANAMNESE] Resultado IA: ok=${resultado.ok} concluida=${resultado.concluida} erro=${resultado.error || 'nenhum'}`);
...
async function buscarOrcAtivo(numero) {
  ...
  console.log(`[BUSCA] Buscando ORC para numero: ${numLimpo}`);
