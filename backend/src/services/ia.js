...
- Mencione sempre que com fotos e detalhes o profissional pode orçar SEM visita.
- Quando tiver todas as informações, responda APENAS: ANAMNESE_CONCLUIDA
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `Você gera resumos concisos de atendimentos. Dado o histórico sobre "${servicoNome}", 
          gere um resumo em 3-5 linhas com: tipo do serviço, local, urgência, disponibilidade do cliente. 
          Seja objetivo e use bullet points simples. Em português.`
        },
        {
          role: 'user',
