...
function extrairIdentificadores(texto) {
  const refMatch = texto.match(/ref-([a-f0-9]+)/i);
  ...
  const servico_id = servicoMatch?.[1] || null;
  const prestador_id = prestadorMatch?.[1] || null;

  return {
    servico_id,
    prestador_id,
    categoria: catMatch?.[1]?.trim() || null,
    temIdentificador: !!(refMatch || servico_id || prestador_id)
  };
}
